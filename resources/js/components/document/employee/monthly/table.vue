            ...
            <el-button
              @click="
                downloadZip(
                  ...
                  scope.row.id,
                  `${scope.row.identification_id}_${
                    scope.row.service.description
                  }_${scope.row.surname.toCamelCase()}_${scope.row.name.toCamelCase()}_all`
                )
              "
              type="info"
...
export default {
  ...
  methods: {
    ...
    edit(id_service, id_employee) {
      window.location.href = `${window.location.origin}/services/${id_service}/documents/employees/${id_employee}/monthly/${this.monthYear}`;
    },
    filterSearch() {
      this.loading = true;
