  ...
  <div>
    ...
    >
      <div slot="modal-title" v-if="!$truthty(message)">
        ¿Estás seguro de notificar al administrador?
      </div>
        ...
        <el-table-column label="Acciones" width="120">
          <template slot-scope="scope">
            <el-button
              v-if="
                auth.user_type_id == 1 ||
                auth.company_id === scope.row.company.id
              "
            ...
            <el-button
              ...
              v-b-tooltip.hover
              title="Notificar al administrador"
              circle
  ...
  methods: {
    filterSearch(value) {
      window.location.href =
        ...
        "/services/documents/employees/monthly/" +
        value;
    },
    ...
    edit(id_service) {
      switch (this.monthly) {
        case 1:
          window.location.href = `${window.location.origin}/services/${id_service}/documents/employees/base`;
          break;
        case 2:
          window.location.href = `${window.location.origin}/services/${id_service}/documents/employees/monthly`;
          break;
        default:
      }
    },
    ...
    submit() {
      ...
      axios
        .post(`${window.location.origin}/mail/documents/load`, {
          documents: [],
          service_id: this.service.id,
          area: 1,
          temp: this.monthly,
          period: this.service.month_year_registry,
        })
        .then((response) => {
