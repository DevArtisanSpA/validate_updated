...
{{`${props.row.service.description.toCamelCase()} tipo ${props.row.service.service_type.name}`}}
        ...
        <!-- <el-table-column
          ...
          prop="rut"
          width="100"
          :sort-method="sortSecundario('rut')"
          :filters="valuesFilter(tableData, 'rut')"
          :filter-method="filterRow('rut')"
        /> -->
        <!-- <el-table-column
        v-if="this.$props.states.temp == 1"
        label="Periodo"
        sortable
        width="120"
        prop="month_year"
        :sort-method="sortTernario('month_year')"
        :filters="valuesFilterMonthYear(tableData)"
...
export default {
  ...
  methods: {
    ...
    goTo(row) {
      window.location.href = `${window.location.origin}/review/${row.service.id}/documents/employees/${row.id}/base`;
