  ...
  <body class="body">
    <div>
      ...
      <div style="text-align: left; ">
        ...
        <p><br>
          Estimados, les informamos que los documentos base de los empleados
          del servicio <strong>{{ strtoupper($service->description) }}</strong> de la
          empresa contratista <strong> {{ strtoupper($service->company->business_name) }}</strong>,
          para la empresa principal <strong>
            {{ strtoupper($service->branchOffice->company->business_name) }} </strong>, sucursal
          <strong> {{ strtoupper($service->branchOffice->name) }}</strong>
          han sido revisados y actualizados en sus estados.
        </p>
        @if($wrong)
        ...
        <ul>
          ...
          <li>
            <h3>{{strtoupper($employee->surname)}}
              @if(!is_null($employee->second_surname))
              {{strtoupper($employee->second_surname)}},
              @endif
